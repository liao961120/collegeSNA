...
class freshmanEgoNetworkSpider(CrawlSpider):
    name = "egoNetwork"
    allowed_domains = ['freshman.tw']
    
    # Select a department as the ego of the network, in this case 臺大圖資
    start_urls =  ['http://freshman.tw/cross/104/001062']
    

    # Rules for Horizontal and Vertical Crawling  follow=True > Recursive Crawl
    rules = (
        Rule(LinkExtractor(restrict_xpaths="//div[@class='card-block']//small"), follow=True, callback='parse_item'),
    )
